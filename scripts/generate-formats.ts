...
// JSON
writeFileSync(join(dataDir, 'slugs.json'), JSON.stringify(reservedSlugsArray, null, 2) + '\n')

// Plain text (one per line)
writeFileSync(join(dataDir, 'slugs.txt'), reservedSlugsArray.join('\n') + '\n')

// CSV (single column with header)
writeFileSync(join(dataDir, 'slugs.csv'), 'slug\n' + reservedSlugsArray.join('\n') + '\n')
...
writeFileSync(join(dataDir, 'slugs.toml'), 'slugs = [\n' + tomlValues.join(',\n') + ',\n]\n')

console.log(`Generated ${reservedSlugsArray.length} slugs in 6 formats to data/`)
